...
export function DashboardWorkspace() {
  const { invoices, total, loadError, realtimeStatus } = useInvoiceSnapshot();
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-[#929eae]">
            Your financial summary is synced from the browser after the initial
            load.
          </p>
        </div>
        <p className="text-sm font-semibold text-[#929eae]">
          {total} invoice{total === 1 ? "" : "s"} in Appwrite · realtime{" "}
          {realtimeStatus}
        </p>
      </div>
